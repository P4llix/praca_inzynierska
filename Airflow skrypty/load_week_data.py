...
dag = DAG(
	'Dodawanie_rekordow',
	description='Pobieranie i wrzucanie danych do bazy',
	schedule='@continuous',
...
def finish_tasks(**kwargs):
	...
	mysql_hook = MySqlHook(mysql_conn_id='aws_mysql')
	mysql_hook.run("update cepik_synchronizacja set ostatnie_sprawdzenie = now(), status = 1 where rowid=" + str(rowid))
